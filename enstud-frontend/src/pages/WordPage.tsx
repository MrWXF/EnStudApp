...
export default function WordPage() {
  ...
  if (studying && words.length > 0 && currentWord) {
    ...
    return (
      <Card title="单词学习">
        ...
        <Card style={{ background: '#fafafa', textAlign: 'center', minHeight: 200 }}>
          <h1 style={{ fontSize: 36, margin: '16px 0' }}>{currentWord.word}</h1>
          {currentWord.phoneticUs && <p style={{ color: '#8c8c8c' }}>/ {currentWord.phoneticUs} /</p>}
          {showAnswer ? (
            <div style={{ marginTop: 16 }}>
              <p style={{ fontSize: 18 }}>{currentWord.definitionCn}</p>
              {currentWord.exampleSentence && (
                <p style={{ color: '#8c8c8c', fontStyle: 'italic' }}>"{currentWord.exampleSentence}"</p>
              )}
      ...
      <Spin spinning={loading}>
        <Space>
          <Select
            ...
            options={wordbooks.map((b) => ({ label: `${b.name} (${b.wordCount}词)`, value: b.id }))}
          />
