...
export default function ChatPage() {
  ...
  const loadSessions = async () => {
    try { const res: any = await getSessions(); setSessions(res.data || []); } catch {}
  };
  const loadMessages = async (id: number) => {
    try { const res: any = await getMessages(id); setMessages(res.data || []); } catch {}
  };

  const handleNewSession = async () => {
    try {
      const res: any = await createSession(scenario);
  ...
  const handleSend = async () => {
    ...
    try {
      const res: any = await sendMessage(activeSession, text);
      const { aiMessage } = res.data;
      setMessages((prev) => [...prev, { role: 'AI', content: aiMessage.content, grammarIssues: aiMessage.grammarIssues }]);
    } catch { message.error('发送失败'); }
    finally { setLoading(false); }
  };

  const handleDelete = async (id: number) => {
    try { await deleteSession(id); loadSessions(); if (activeSession === id) setActiveSession(null); }
  ...
  return (
    <div style={{ display: 'flex', gap: 16, height: 'calc(100vh - 140px)' }}>
      <Card style={{ width: 240, overflow: 'auto' }} size="small" title={
        <Space><Select value={scenario} onChange={setScenario} size="small" style={{ width: 100 }}
          options={[
            { label: '自由对话', value: 'FREE' }, { label: '日常', value: 'DAILY' },
            { label: '商务', value: 'BUSINESS' }, { label: '旅行', value: 'TRAVEL' },
          ]} />
        ...
        <List size="small" dataSource={sessions} renderItem={(s: any) => (
          <List.Item style={{ cursor: 'pointer', background: activeSession === s.id ? '#e6f4ff' : undefined }}
            onClick={() => setActiveSession(s.id)}
            actions={[<Button size="small" type="text" danger icon={<DeleteOutlined />} onClick={(e) => { e.stopPropagation(); handleDelete(s.id); }} />]}
          >
        ...
        <div ref={listRef} style={{ flex: 1, overflow: 'auto', padding: 16 }}>
          {messages.map((m, i) => (
            <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 16, flexDirection: m.role === 'USER' ? 'row-reverse' : 'row' }}>
              <Avatar icon={m.role === 'USER' ? <UserOutlined /> : <RobotOutlined />} style={{ background: m.role === 'USER' ? '#1677ff' : '#52c41a' }} />
              <div style={{ maxWidth: '70%', background: m.role === 'USER' ? '#1677ff' : '#f0f0f0', color: m.role === 'USER' ? '#fff' : '#000', padding: '8px 16px', borderRadius: 12, whiteSpace: 'pre-wrap' }}>
                ...
                {m.grammarIssues && m.grammarIssues.length > 0 && (
                  <div style={{ marginTop: 8 }}>
                    {m.grammarIssues.map((g: any, j: number) => (
